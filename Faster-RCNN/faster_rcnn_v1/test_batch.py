...
def visualize_and_save_predictions(image, predictions, output_path, threshold=0.5):
...
def test_model_on_directory(model_path, test_image_dir, output_dir, device):
    """Tests the model on images from a directory and saves the output images with predictions.

    Args:
        model_path (str): Path to the saved model checkpoint.
        test_image_dir (str): Directory containing test images.
        output_dir (str): Directory to save output images with predictions.
        device (torch.device): Device to use (CPU or GPU).
    """
...
if __name__ == "__main__":
    ...
    test_image_dir = "/data/malaria_proj/building_data/unification/models/Nigeria_test_data"
    output_dir = "/data/malaria_proj/building_data/unification/models/test_outputs_nigera"

    # Path to the saved model
    model_path = 'model_checkpoint.pth'

    # Run the testing
    test_model_on_directory(model_path, test_image_dir, output_dir, device)
